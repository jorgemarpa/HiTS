...
def give_me_lc(field, CCD, X, Y, extract=False):
    year = field[:-3]
    # try:
    tar = tarfile.open("%s/lightcurves/%s/%s/%s_%s_LC_50.tar.gz"
...
def GP_fit(time, mag, err, x_pred=None, plot=True):
    ...
    gp = GaussianProcessRegressor(kernel=kernel, alpha=(err)**2,
...
def HiTS_depth_noise(mag):
    ...
    # scale = np.random.uniform(15., 18.)
    btw = empirical.query('gMedianKronMag > %f-.05 and gMedianKronMag < \
                          %f+.05 and gMedianKronMagErr < .5' % (scale, scale))
    errors = np.random.choice(btw.gMedianKronMagErr, size=len(mag))
...
def calculate_FATS_features(id_lc, lc_g, verbose=True):
    ...
    time, mag, error = np.array(lc_g['MJD'].values), \
                       np.array(lc_g['MAG_KRON'].values), \
...
def P4J_period(lc_g):
    ...
    if True:
        ...
        model_ = P4J.periodogram(method='QMIEU')
        model_.set_data(time, mag-np.mean(mag), error)
        model_.frequency_grid_evaluation(fmin=1/30., fmax=1/.0035,
                                         fresolution=0.05)
        model_.finetune_best_frequencies(fresolution=0.01, n_local_optima=10)

        fbest, pbest = model_.get_best_frequencies()
...
def preprocess_lc(lc, min_mag=15.):
    ...
    # sigma clipping
    filtered_data = sigma_clip(np.array(lc['MAG_KRON'].values),
                               sigma=4, iters=1, cenfunc=np.mean, copy=False)
...
def run_code(var_class='RRLYR', sample=50, n_gen=20,
             n_process=3, plot=False):
    ...
    for item in class_example.iterrows():
        ...
        if var_class == 'NV':
            ...
            mag_obs = GP_fit_periodic(time, mag, err, T, T_1=T,
                                      plot=plot, x_pred=hits_time)
        ...
        if plot:
            if var_class == 'NV':
                plt.figure(figsize=(9, 4))
                plt.errorbar(hits_time, hits_mag, yerr=hits_err, fmt='k.',
                             ms=7, lw=1, alpha=1)
                plt.errorbar(hits_time, hits_noise_mag, yerr=hits_err,
                             fmt='r.', ms=7, lw=1, alpha=1)
                plt.gca().invert_yaxis()
                plt.show()
                ...
                hits_noise_MAG = np.concatenate([hits_noise_MAG,
                                                 hits_noise_MAG])
                hits_PHASE = np.concatenate([hits_PHASE, hits_PHASE+1])
                hits_ERR = np.concatenate([hits_ERR, hits_ERR])

                plt.errorbar(hits_PHASE, hits_MAG, yerr=hits_ERR, fmt='k.',
                             ms=7, lw=1, alpha=1)
                plt.errorbar(hits_PHASE, hits_noise_MAG, yerr=hits_ERR,
                             fmt='r.', ms=7, lw=1, alpha=1)
                plt.gca().invert_yaxis()
                plt.show()
        ...
        for k in range(n_gen):
            ...
            if var_class == 'NV':
                ...
                mag_obs = GP_fit_periodic(time, mag, err, T, T_1=T_1,
                                          plot=False, x_pred=hits_time)
            # scale to HiTS depth and add empirical uncertainties
            hits_mag, hits_noise_mag, hits_err = HiTS_depth_noise(mag_obs)
            if hits_mag.mean() < 20:
                size = int(len(hits_time) * np.random.uniform(.95, 1., size=1))
            ...
            df = pd.DataFrame(np.array([hits_time,
                                        hits_noise_mag,
                                        hits_err]).T,
                              columns=['MJD', 'MAG_KRON', 'MAGERR_KRON'])
            if False:
                path = '%s/synt_lcs/%s_%i_%.6f.csv' % (mainpath,
                                                       var_class, k, T_1)
                df.to_csv(path)
            all_lcs.append(df)
            all_per.append(T_1)
...
if __name__ == '__main__':
    ...
    run_code(var_class=clase, sample=use, n_gen=total, n_process=cores,
             plot=False)
