...
def run_lc(field, CCD, FILTER, row_pix, col_pix):
	...
	if len(time_series) > 0 or len(time_series_no) > 0:
		...
		ax[0].invert_yaxis()
		ax[1].invert_yaxis()
		plt.savefig('%s/%s/%s/%s_lightcurve.png' % (webpath, field, CCD, name), dpi = 300)
	...
	if len(images) > 0:
		...
		plt.savefig('%s/%s/%s/%s_sequence.png' % (webpath, field, CCD, name), dpi = 300, bbox_inches = 'tight', pad_inches = 0.3)
...
if __name__ == '__main__':
	...
	for CCD in CHIPS:
		...
		ang0 = np.array(ang)
		distance.append(ang0)
		print 'Nearest nthneighbor at %f arcsec' % (ang0*3600)
	...
	try:
		...
		print 'is empty!'
		print '\nNo matched object within %f arcsec' % (tolerance)
	print '_________________________________________________________'
