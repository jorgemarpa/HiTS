...
# simple class to compute RA DEC or i j given CRPIX, CRVAL, CD, PV
class WCS(object):

    def __init__(self, CRPIX, CRVAL, CD, PV):
    ...
    def RADEC(self, i, j):
        ...
        x = self.CD11 * (i - self.CRPIX1) + self.CD12 * (j - self.CRPIX2) # deg 
        y = self.CD21 * (i - self.CRPIX1) + self.CD22 * (j - self.CRPIX2) # deg
        r = np.sqrt(x**2 + y**2) # deg
        xi = deg2rad * (self.PV[0, 0] + self.PV[0, 1] * x + self.PV[0, 2] * y + self.PV[0, 3] * r + self.PV[0, 4] * x**2 + self.PV[0, 5] * x * y + self.PV[0, 6] * y**2 + self.PV[0, 7] * x**3 + self.PV[0, 8] * x**2 * y + self.PV[0, 9] * x * y**2 + self.PV[0, 10] * y**3)
        eta = deg2rad * (self.PV[1, 0] + self.PV[1, 1] * y + self.PV[1, 2] * x + self.PV[1, 3] * r + self.PV[1, 4] * y**2 + self.PV[1, 5] * y * x + self.PV[1, 6] * x**2 + self.PV[1, 7] * y**3 + self.PV[1, 8] * y**2 * x + self.PV[1, 9] * y * x**2 + self.PV[1, 10] * x**3) # rad
        fu = xi / np.cos(self.CRVAL2 * deg2rad) # rad
        fv = 1. - eta * np.tan(self.CRVAL2 * deg2rad) # rad
        fw = eta + np.tan(self.CRVAL2 * deg2rad) # rad
        fa = np.arctan2(fu, fv) # rad
        RA = self.CRVAL1 + fa * rad2deg # deg
        DEC = np.arctan2(fw * np.cos(fa), fv) * rad2deg # deg

        return (RA, DEC)
        
    # inverse RA DEC, numeric solution using Newton method
    def ij(self, RA, DEC, i0, j0):
        ...
        err = 1.
        while err > 1e-5:
            RAi, DECi = self.RADEC(ij[0], ij[1])
            dRAi = (self.RADEC(ij[0] + delta, ij[1])[0] - RAi) / delta
            dRAj = (self.RADEC(ij[0], ij[1] + delta)[0] - RAi) / delta
            dDECi = (self.RADEC(ij[0] + delta, ij[1])[1] - DECi) / delta 
            dDECj = (self.RADEC(ij[0], ij[1] + delta)[1] - DECi) / delta
...
# general class to find astrometric solution using analytic jacobian and minimizer
class WCSsol(object):

    CDscale = 1e-5 #0.27 / 60. / 60. / 4. # used for chi2 minimization only
...
class starmatch(object):
    ...
    def __init__(self, RA, DEC, mag, i, j, flux, r, flag, CRPIX, CRVAL, CD, PV, doplot, HiTSpath="/home/apps/astro/HiTS", field="test", CCD="test", epoch=0):
    ...
    def match(self):
        ...
        npix = 100
        nstars = 60
        masksex = (self.flag == 0) & (self.r < np.percentile(self.r, 90)) & (self.flux > np.percentile(self.flux, 10))
        ...
        if self.doplot:
            try:
                ...
                plt.savefig("%s/WEB/TESTING/fforster/astrometry/%s_%s_%02i_stars.png" % (self.HiTSpath, self.field, self.CCD, self.epoch))
            except:
        ...
        if self.doplot:
            ...
            plt.savefig("%s/WEB/TESTING/fforster/astrometry/%s_%s_%02i_stars_delta.png" % (self.HiTSpath, self.field, self.CCD, self.epoch))
        ...
        if self.doplot:
            ...
            plt.savefig("%s/WEB/TESTING/fforster/astrometry/%s_%s_%02i_stars_corrected.png" % (self.HiTSpath, self.field, self.CCD, self.epoch))
        ...
        if self.doplot:
            ...
            plt.savefig("%s/WEB/TESTING/fforster/astrometry/%s_%s_%02i_stars_match.png" % (self.HiTSpath, self.field, self.CCD, self.epoch))
...
if __name__ == "__main__":
    ...
    print "test" # this is needed to make this code work from josepena's account! WHY???
    ...
    from look import lookUSNOESO

    # find public reference catalogue
    def getUSNO(field, CCD, epoch, NAXIS, CRPIX, CRVAL, CD, PV):
    
        # first look for public catalogue of stars if necessary
        USNOfile = "%s/SHARED/%s/%s/CALIBRATIONS/USNO_%s_%s_%02i.npy" % (HiTSpath, field, CCD, field, CCD, epoch)
        if os.path.exists(USNOfile):
            #print "Found USNO file"
            USNO = np.load(USNOfile)
        else: # get central coordinates, query data and save
            print "   Cannot find USNO file %s, querying remote server..." % USNOfile
            WCS = WCSsol(0, 0, NAXIS[0] / 2., NAXIS[1] / 2., CRPIX, CRVAL, CD, PV)
            USNO = lookUSNOESO(WCS.RA() / 15., WCS.DEC(), 10, True)
            if not (USNO is None):
                if not os.path.exists("%s/%s/%s/CALIBRATIONS" % (sharedir, field, CCD)):
                    os.makedirs("%s/%s/%s/CALIBRATIONS" % (sharedir, field, CCD))
                np.save(USNOfile, USNO)
        ...
        return (RAUSNO, DECUSNO, magUSNO)

    # solve astrometry
    def doastrometry(field, CCD, epoch, dofilter):

        filename = "%s/DATA/%s/%s/%s_%s_%02i_image_crblaster.fits" % (HiTSpath, field, CCD, field, CCD, epoch)
        ...
        CRVAL = np.zeros(2)
        for i in range(2):
            NAXIS[i] = float(header["NAXIS%i" % (i + 1)])
        ...
        #DECUSNO = np.array(USNO[1], dtype = float)
        #magUSNO = np.array(USNO[3], dtype = float)
        (RAUSNO, DECUSNO, magUSNO) = getUSNO(field, CCD, epoch, NAXIS, CRPIX, CRVAL, CD, PV)
        ...
        # sextractor file
        sexfile = "%s/SHARED/%s/%s/%s_%s_%02i_image_crblaster.fits-catalogue_wtmap_backsize64.dat" % (HiTSpath, field, CCD, field, CCD, epoch)
        ...
        print "Running minimization routine on epoch %i..." % epoch
        ...
        if gcsol.success:
            ...
            pickle.dump(astrosol, open("%s/SHARED/%s/%s/CALIBRATIONS/astrometry_%s_%s_%02i.pkl" % (HiTSpath, field, CCD, field, CCD, epoch), 'wb'), protocol = pickle.HIGHEST_PROTOCOL)
            ...
            #plt.savefig("%s/WEB/TESTING/fforster/astrometry/%s_%s_%02i_stars_finalsolution.png" % (HiTSpath, field, CCD, epoch))

            return sol

    # Field, CCD and epoch to process
    HiTSpath = "/home/apps/astro"
    sharedir = "%s/SHARED" % HiTSpath
    ...
    for ifield in range(nfields):
        ...
        for h in ["N", "S"]:
            for nCCD in range(31):
                ...
                files = os.listdir("%s/DATA/%s/%s" % (HiTSpath, field, CCD))
                for filei in sort(files):

                    if re.match("%s_%s_(\S+)_image_crblaster.fits" % (field, CCD), filei):
                        epoch = int(re.findall("%s_%s_(\S+)_image_crblaster.fits" % (field, CCD), filei)[0])
                        #if epoch != 3:
                        #    continue
                        #print epoch
                        ...
                        sol = doastrometry(field, CCD, epoch, False)
                        #except:
                        #    print "-----> WARNING: Cannot process field %s, CCD %s, epoch %i" % (field, CCD, epoch)
                        #    continue
